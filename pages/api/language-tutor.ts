    ...
    switch (action) {
      ...
      case "vocabulary":
        try {
          ...
          return res
            .status(500)
            .json({ error: "Failed to generate word exercises" });
        }
      case "grammar":
        try {
          ...
          return res
            .status(500)
            .json({ error: "Failed to generate grammar exercise" });
        }
      default:
...
async function generateWordExercises(
  skillLevel: string,
  count: number = 5
): Promise<WordExercise[]> {
...
function createDefaultExercise(skillLevel: string): GrammarExercise {
  switch (skillLevel) {
    ...
    case "Advanced":
      return {
        question:
          "Select the correct conditional form: If I ___ about the exam, I would have studied more.",
